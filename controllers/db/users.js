...
module.exports = function (knex, req, res, next) {
    ...
    q.leftJoin('user_skills', 'id', '=', 'user_id').select(knex.raw("array_agg((skill_id, count)) AS skills"))
        .groupBy('id').limit(req.body.limit || 20).offset(req.body.offset || 0).then(function(rows) {
            if (!rows) res.end();
