...
module.exports = function (skills) {
    ...
    this.leaves = []; //хранит ——џЋ » (не id) на все листь€
    this.skills = {}; //объект, в котором свойствами €вл€ютс€ скиллы

    for (var id in skills) {
        //копируем имеющиес€ свойства
        this.skills[id] = this.skills[id] || {}; //проверка нужна, потому что дальше объекты могут создаватьс€
        this.skills[id].title = skills[id].title;
        this.skills[id].parents = []; //хранит ——џЋ » (не id) на родителей
        ...
        //создаем массив ——џЋќ  (не id) на детей
        this.skills[id].children = this.skills[id].children || []; //проверка нужна, чтобы children сохран€лись
        for (i = 0; i < this.skills[id].parents.length; i++) {
    ...
    //рекурси€ дл€ добавлени€ всех детей вплоть до листьев и всех листьев
    ...
    //рекурси€ дл€ добавлени€ всех родителей вплоть до корн€
